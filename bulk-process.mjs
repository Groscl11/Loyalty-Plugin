...
/**
 * Bulk reprocess unprocessed orders using Node.js
 */
...
async function processOrder(orderNumber) {
  try {
    ...
    if (orderError || !order) {
      console.error(`❌ Order ${orderNumber} not found:`, orderError?.message || 'Not found');
      return;
    }
    ...
    if (existingMembers && existingMembers.length > 0) {
      ...
      if (createError) {
        console.error(`❌ Error creating member:`, createError.message);
        return;
      }
    ...
    if (statuses && statuses.length > 0) {
      ...
      if (createStatusError) {
        console.error(`❌ Error creating loyalty status:`, createStatusError.message);
        return;
      }
